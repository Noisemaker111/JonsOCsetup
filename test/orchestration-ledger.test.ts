...
test("spawn records the reasoning effort and fast flag from the worker identity, and it survives to the tracked session row", () => {
  const { dir, file } = fixture()
  const ev = { sessionID: "ses_parent", callID: "call-1", input: {}, workerIdentity: { agentRole: "worker", providerID: "cliproxyapi", modelID: "gpt-5.6-sol", runtime: "native", parentID: "ses_parent", runID: "call-1", task: "x", reasoningEffort: "xhigh", fast: true } }
  recordSpawn(ev, file)
  const row = trackedChildren("ses_parent", { running: [], recent: [] }, file)[0]
  expect(row).toMatchObject({ reasoningEffort: "xhigh", fast: true })
  rmSync(dir, { recursive: true, force: true })
})

test("the terminal notification carries the same reasoning effort and fast flag recorded at spawn", () => {
  const { dir, file } = fixture()
  const ev = { sessionID: "ses_parent", callID: "call-1", input: {}, workerIdentity: { agentRole: "worker", providerID: "openai", modelID: "gpt-5.6-luna-fast", runtime: "native", parentID: "ses_parent", runID: "call-1", task: "x", reasoningEffort: "medium", fast: true } }
  recordSpawn(ev, file)
  appendLedger({ kind: "bound", parentID: "ses_parent", callID: "call-1", childID: "ses_child", openCodeSessionId: "ses_child", runtime: "native" }, file)
  recordNotification("ses_parent", "call-1", "ses_child", "completed", "done", file)
  const completion = pendingCompletionEvidence("ses_parent", file)[0]!
  expect(completion).toMatchObject({ reasoningEffort: "medium", fast: true })
  rmSync(dir, { recursive: true, force: true })
})
...
test("orchestration review flow messages the same integration session", () => {
  ...
  expect(giver).toContain("Runtime handles dispatch, workspace and route policy")
  // The giver has no shell: review is a worker step that loads skills/review.
