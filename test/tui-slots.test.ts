...
const HOST_EXE = resolveHostExecutable()
...
test.skipIf(!existsSync(HOST_EXE))("the running host still renders exactly these slots", async () => {
  ...
  // calls. The JSX factory name is minified and changes across host builds.
  const text = new TextDecoder("latin1").decode(await Bun.file(HOST_EXE).bytes())
  const found = [...text.matchAll(/[A-Za-z_$][\w$]*\([A-Za-z$_][\w$]*,\{path:"([a-z][a-z.]*)"/g)].map((m) => m[1])
