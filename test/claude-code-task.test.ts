...
import { classifyFailure, classifyStreamResult, parseClaudeStreamResult, parseClaudeStreamText, claudeCodeTaskTool, claudeCodeTool, discoverClaudeRuntime, extractSessionId, installClaudeCodeIntercept, interceptClaudeCodeTask, isClaudeCodeSpawn, makeClaudeCodeTaskTool, questFaceTitle, resolveClaudeCodeWorkerName, runClaudeCodeTask } from "../plugins-active/claude-code-task"
...
test("questFaceTitle is the live line for a ledger-bound run and the model-task title otherwise", async () => {
  const { QuestStore } = await import("../quest/store")
  const dir = mkdtempSync(join(tmpdir(), "cc-face-"))
  const prior = process.env.OPENCODE_QUEST_ROOT
  try {
    const quest = new QuestStore(dir).create({ id: "01j00000000000000000000cc1", title: "Claude Quest", objective: "Face the chip" })
    process.env.OPENCODE_QUEST_ROOT = dir
    expect(questFaceTitle(quest.id, { providerID: "claude-code", modelID: "sonnet", task: "do the work" }))
      .toBe("(Claude Quest, claude-code/sonnet)")
    expect(questFaceTitle("unbound", { providerID: "claude-code", modelID: "sonnet", task: "do the work" }))
      .toBe("claude-code/sonnet - do the work")
    expect(questFaceTitle("01j00000000000000000000miss", { providerID: "claude-code", modelID: "sonnet", task: "do the work" }))
      .toBe("claude-code/sonnet - do the work")
  } finally {
    if (prior === undefined) delete process.env.OPENCODE_QUEST_ROOT
    else process.env.OPENCODE_QUEST_ROOT = prior
    rmSync(dir, { recursive: true, force: true })
  }
})

test("a quest-bound run reports the live line as its result title and label", async () => {
  await withIsolatedSessionState(async () => {
    const { QuestStore } = await import("../quest/store")
    const dir = mkdtempSync(join(tmpdir(), "cc-face-e2e-"))
    const prior = process.env.OPENCODE_QUEST_ROOT
    const directory = await mkdtemp(join(tmpdir(), "opencode-claude-face-"))
    const fake = join(directory, "fake-claude.mjs")
    await writeFile(fake, `console.log(JSON.stringify({ type: "result", result: "ok" }))`)
    try {
      const quest = new QuestStore(dir).create({ id: "01j00000000000000000000cc2", title: "Claude Quest", objective: "Face the chip" })
      process.env.OPENCODE_QUEST_ROOT = dir
      const boundScope = { ...scope, questId: quest.id, modelPin: "claude-code/sonnet" }
      const result = await (makeClaudeCodeTaskTool() as any).execute({ task: "do the full task", cwd: directory, scope: boundScope, model: "sonnet", executable: process.execPath, executableArgs: [fake] })
      expect(result.metadata.title).toBe("(Claude Quest, claude-code/sonnet)")
      expect(result.metadata.label).toBe("(Claude Quest, claude-code/sonnet)")
    } finally {
      if (prior === undefined) delete process.env.OPENCODE_QUEST_ROOT
      else process.env.OPENCODE_QUEST_ROOT = prior
      rmSync(dir, { recursive: true, force: true })
...
test("harness ledger keeps parent lineage and runtime session metadata", () => {  const file = join(tmpdir(), `opencode-ledger-${randomUUID()}.jsonl`)
