...
test('nudge retains the selected board before asynchronous prompt navigation',async()=>{
 const f=fixture();try{const {nudgeGiver,rememberBoardView,returnToQuest}=await import('../quest/tui-workflow');const routes:any[]=[];let current:any={type:'plugin',id:'quests',name:'quests',data:{returnRoute:{type:'session',sessionID:'ses_giver'}}};const context:any={ui:{router:{current:()=>current,navigate:(r:any)=>{routes.push(r);current=r}},dialog:{prompt:async()=>'Inspect this Quest',clear:()=>{}}},client:{session:{get:async()=>({id:'ses_giver',location:{directory:f.root}}),prompt:async()=>{current={type:'session',sessionID:'ses_giver'};rememberBoardView(context,{questID:'other',filter:'attention',allProjects:true})}}}};rememberBoardView(context,{questID:f.q().id,filter:'all',allProjects:false,projectDirectory:f.root});await nudgeGiver(context,f.q());returnToQuest(context);expect(routes[0]).toEqual({type:'session',sessionID:'ses_giver'});expect(routes[1].type).toBe('plugin');expect(routes[1].data.questID).toBe(f.q().id);expect(routes[1].data.projectDirectory).toBe(f.root)}finally{f.close()}
})
