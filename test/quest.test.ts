...
import { deriveState } from "../quest/state-machine"
import { questLane } from "../quest/board"
import { questIndicator, questLaneCounts } from "../quest/tui-model"
...
function doneGatedQuest(id: string, sessionState: "completed" | "executing"): Quest {
  return {
    ...newQuest({ id, title: "Done gated", objective: "x" }),
    state: "Waiting",
    stages: [{
      id: "step-one", title: "Do it", status: "done", needs: [],
      todos: [{ id: "t1", title: "did it", status: "done" }],
      claim: { repos: [], include: [], exclude: [] },
      proofs: [{ id: "p1", kind: "command", at: "2026-09-04T21:00:00.000Z", attempt: 1, result: "passed", command: "bun test quest" }],
      attempt: 1,
    }],
    sessions: [{ callID: "call1", role: "worker", state: sessionState, evidence: [], deliverables: [], attempt: 1, updatedAt: "2026-09-04T21:00:00.000Z" }],
    evidence: { commits: [], tests: [{ command: "bun test quest", result: "passed", at: "2026-09-04T21:00:00.000Z" }], builds: [], artifacts: [], publish: [] },
  }
}
test("done-but-gated quests derive Verifying, not assigned; live work stays Working; active counts only executing workers", () => {
  const gated = doneGatedQuest("01j00000000000000000000010", "completed")
  expect(completionMissing(gated).every((m) => m.startsWith("VERIFY-LIVE"))).toBe(true)
  expect(deriveState(gated).state).toBe("Verifying")
  expect(questLane({ ...gated, state: deriveState(gated).state })).toBe("verifying")
  const live = doneGatedQuest("01j00000000000000000000011", "executing")
  expect(deriveState(live).state).toBe("Working")
  const attention = { ...newQuest({ id: "01j00000000000000000000012", title: "Blocked", objective: "x" }), state: "Needs attention" as const }
  expect(questLaneCounts([gated, live, attention])).toMatchObject({ active: 1, attention: 1, verifying: 0, waitingNew: 1 })
  expect(questLaneCounts([{ ...gated, state: "Verifying" }, live, attention])).toMatchObject({ active: 1, attention: 1, verifying: 1 })
  expect(questIndicator([{ ...gated, state: "Verifying" }, live, attention])).toBe("0 to turn in, 1 active, 1 need attention, 1 verifying, 0 waiting/new")
})
