...
async function createQuestFixture(sourceRoot: string, project: string, ledger: string) {
  ...
  const api=questsAPI(store,{project:projectIdentity(project),sessionID:"fixture",requestID:"capture"},async()=>{throw Error("Visual fixture never dispatches")})
  const quest=api.create({title:"Visual E2E Quest",description:"Inspect the real isolated terminal",steps:[{id:"capture",title:"Capture the Quest view"}],reward:"Use /quests to review steps and collected changes."})
  return {store,id:quest.id,project:projectIdentity(project)}
}
...
async function runScenario(root: string, scenario: VisualScenario, out: string, keepRun: boolean, probe: boolean, cliPlugins = false): Promise<Artifact> {
  ...
  const cliBootstrap = join(root, "tui-bootstrap", scenario)
  ...
  // Optional frozen numeric evidence for real /usage acceptance; never read credentials.
  const usageEvidence=option('--usage-evidence-cache')
  if(scenario==='usage'&&usageEvidence){
    const snapshot=resolve(usageEvidence)
    for(const name of ['account-usage.json','account-usage.json.observations','requests.jsonl'])copyFileSync(join(snapshot,name),join(run,name))
  }
  ...
  try {
    ...
    if (probe) {
      ...
      await enter(200)
      captured = await waitForScreen(render, text => text.includes("Subscription usage") && (text.includes("points remaining") || text.includes("No cached accounts")), 15_000, "/usage evidence dialog")
      writeScreenshot(captured.frame,captured.text,join(out,"usage-initial"),"OpenCode2 usage initial")
      if(usageEvidence){
        for(let i=0;i<8&&!/^\s+openai ·/m.test(captured.text);i++){
          const previousAccount=captured.text.split('\n').find(line=>/^\s+(openai|claude|opencode-go) ·/.test(line))
          await clickText('[a] Account')
          captured=await waitForScreen(render,text=>text.split('\n').some(line=>/^\s+(openai|claude|opencode-go) ·/.test(line)&&line!==previousAccount),5000,'account selection repaint')
        }
        if(!/^\s+openai ·/m.test(captured.text))throw Error('OpenAI account selection did not settle')
        await sleep(500);captured=await render();
        writeFileSync(join(out,'account-settled.txt'),captured.text);
        await send('v',150)
        captured=await waitForScreen(render,text=>/^\s+openai ·/m.test(text)&&text.includes('Reset period')&&text.includes('Used points'),5000,'actual quota history')
        writeScreenshot(captured.frame,captured.text,join(out,'usage-history'),'OpenCode2 usage history')
        await send('v',150)
        captured=await waitForScreen(render,text=>/^\s+openai ·/m.test(text)&&text.includes('Request counters'),5000,'actual request evidence')
        await send('n',150);captured=await render()
      }
    } else {
      // The board no longer opens itself: the host composer is the primary surface. Open it the way Jk does.
      if(has("--workflow")) {
        await waitForScreen(render,text=>text.includes("Quests · project 1"),10000,"scoped compact Quest count")
        await clickText("Quests · project")
        await waitForScreen(render,text=>text.includes("Quest counts · current project"),5000,"all compact counts reachable")
        await enter(300)
      } else { await paste("/quests", 200); await enter(300) }
      captured = await waitForScreen(render, (text) => text.includes("Visual E2E Quest") && text.includes("Capture the Quest view"), 15_000, "Quest board")
      writeScreenshot(captured.frame,captured.text,join(out,"quests-initial"),"Quest before recorded recovery fixture")
      if(questFixture){const {store,id}=questFixture;store.apply(id,"session-planned",{callID:"fixture-first",role:"worker",attempt:1,model:"fixture/recorded"});store.apply(id,"session-state",{callID:"fixture-first",state:"failed",result:"Fixture route temporarily unavailable"});store.apply(id,"session-planned",{callID:"fixture-retry",role:"worker",attempt:2,resumedFrom:"fixture-first",resumeRoot:"fixture-first",model:"fixture/recorded"});store.apply(id,"session-state",{callID:"fixture-retry",state:"completed",result:"Fixture recovered on attempt 2",routingNote:"Configured fallback primary → alternative: quota unavailable"})}
      await clickText("Check progress")
      await waitForScreen(render,text=>text.includes("Check progress · recorded runs"),5000,"keyboard/mouse agent log")
      await enter(200)
      await waitForScreen(render,text=>text.includes("Failure reason / run details"),5000,"run inspection actions")
      await enter(200)
      captured=await waitForScreen(render,text=>text.includes("Fixture route temporarily unavailable")&&text.includes("not confirmed"),5000,"historical failure reason and missing session")
      writeScreenshot(captured.frame,captured.text,join(out,"quests-failure-details"),"Recorded failure cause")
      await send("\x1b",200)
      if(has("--workflow")) {
        await clickText("Create giver session")
        await waitForScreen(render,text=>text.includes("Create and explicitly bind"),5000,"explicit giver binding confirmation")
        await clickText("Confirm")
        captured=await waitForScreen(render,text=>text.includes("Return to Quest")&&!text.includes("[q] Select Quest"),10000,"new verified giver conversation")
        const owner=questFixture!.store.read(questFixture!.id)!.integrationOwner
        if(!owner?.startsWith("ses_"))throw Error("Host did not persist confirmed giver identity")
        writeFileSync(join(out,"giver-binding.json"),JSON.stringify({questID:questFixture!.id,sessionID:owner},null,2))
        writeScreenshot(captured.frame,captured.text,join(out,"quests-giver"),"Actual giver conversation")
        await clickText("Return to Quest")
        await waitForScreen(render,text=>text.includes("[g] Talk to Quest Giver"),5000,"return route to Quest")
        await send("g",300)
        await waitForScreen(render,text=>text.includes("Return to Quest")&&!text.includes("[q] Select Quest"),5000,"keyboard Talk to verified giver")
        await clickText("Return to Quest")
        await waitForScreen(render,text=>text.includes("[g] Talk to Quest Giver"),5000,"second giver return")
        await send("s",200)
        await waitForScreen(render,text=>text.includes("Start worker session"),5000,"start session input")
        await send("\x1b",200)
        if(questFixture!.store.read(questFixture!.id)!.sessions.length!==2)throw Error("Dismissed start created a run")
        await send("s",200);await paste("missing-route/not-authorized#medium",150);await enter(300)
        await waitForScreen(render,text=>text.includes("Explicit model must resolve"),10000,"exact invalid route rejection")
        const rejected=questFixture!.store.read(questFixture!.id)!.sessions.at(-1)!
        if(rejected.state!=="failed"||rejected.model!=="missing-route/not-authorized#medium"||rejected.sessionID)throw Error("Start action changed route or fabricated session")
        await send("n",200)
        await waitForScreen(render,text=>text.includes("Nudge Quest Giver"),5000,"nudge input")
        await paste("Reply with the isolated validation receipt. Do not dispatch work.",150);await enter(300)
        await waitForScreen(render,text=>text.includes(responseFixture!.receipt),15000,"real host nudge reaches existing giver")
        writeFileSync(join(out,"nudge-receipt.json"),JSON.stringify({receipt:responseFixture!.receipt,requests:responseFixture!.requests.count,accountInference:false},null,2))
        await clickText("Return to Quest")
        await waitForScreen(render,text=>text.includes("[g] Talk to Quest Giver"),5000,"nudge return")
        await clickText("0 Turn in")
        await waitForScreen(render,text=>text.includes("0 matching"),5000,"count reaches matching records")
        await send("f",200)
        await waitForScreen(render,text=>text.includes("Quest state filter"),5000,"keyboard state filter")
        await clickText("Open ·")
        await waitForScreen(render,text=>text.includes("Visual E2E Quest"),5000,"open state restores Quest")
        const commandQuest=questFixture!.store.read(questFixture!.id)!
        questFixture!.store.apply(commandQuest.id,"patched",{stages:commandQuest.stages.map(s=>({...s,commandID:"fixture"}))})
        await send("s",200)
        await waitForScreen(render,text=>text.includes("completed. Open Check progress"),15000,"actual configured command through Start action")
        const completed=questFixture!.store.read(questFixture!.id)!
        if(completed.stages[0].status!=="done"||completed.sessions.at(-1)?.state!=="completed"||completed.sessions.at(-1)?.agentRole!=="command")throw Error("Configured command did not settle correctly")
        writeFileSync(join(out,"command-receipt.json"),JSON.stringify({questID:completed.id,runID:completed.sessions.at(-1)?.runID,state:completed.sessions.at(-1)?.state,artifacts:completed.evidence.artifacts,accountInference:false},null,2))
        await waitForScreen(render,text=>text.includes("1 Turn in"),10000,"done steps enable turn in")
        await send("t",200)
        await waitForScreen(render,text=>text.includes("Have you reviewed the reward"),5000,"turn-in requires explicit acceptance")
        await send("\x1b",200)
        if(questFixture!.store.read(questFixture!.id)!.archive)throw Error("Dismissed acceptance archived the Quest")
        await send("t",200);await clickText("Confirm")
        await waitForScreen(render,text=>text.includes("1 Archived"),5000,"accepted Quest archived")
        await clickText("1 Archived")
        captured=await waitForScreen(render,text=>text.includes("Reopen Quest"),5000,"archived record remains reachable")
        if(!questFixture!.store.read(questFixture!.id)!.archive?.accepted)throw Error("Explicit acceptance was not recorded")
        writeScreenshot(captured.frame,captured.text,join(out,"quests-accepted"),"Explicitly accepted Quest")
      }
      // Jk could not leave the board: Esc was never bound. Prove it on the real host.
      await send("\x1b", 300)
      await waitForScreen(render, (text) => !text.includes("Visual E2E Quest"), 10_000, "Esc leaves the Quest board")
    }
