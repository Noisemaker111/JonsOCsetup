/** Deterministic create/read/complete goal and read-only audit through Codex.
 * --installed uses ordinary plugin discovery and existing trust without overrides.
 * --hot-activation uses a disposable copy to prove next-call reload in one process.
 * --fixture-skills isolates hook behavior from the separate Windows sandbox ACL issue.
...
if(!installed)cpSync(source,candidate,{recursive:true});if(!installed&&process.argv.includes('--hot-activation'))cpSync('.visual-e2e/installed-backup/hook.js',join(candidate,'scripts/hook.js'))
...
const initial=coordination(store,owner)({action:'status'}),beforeProject=initial.project.id,before=JSON.parse(readFileSync(join(store.runtime,'coordination',beforeProject+'.json'),'utf8')).participants[0]
if(endedOwner){
...
async function hookTrust(){
 ...
 try{await rpc('initialize',{clientInfo:{name:'quest-hook-acceptance',version:'1'},capabilities:{experimentalApi:true}});p.stdin.write(JSON.stringify({method:'initialized',params:{}})+'\n');const cfg=await rpc('config/read',{cwd:repo,includeLayers:true});writeFileSync(join(root,'effective-config-summary.json'),JSON.stringify({plugins:cfg.result?.config?.plugins,projects:cfg.result?.config?.projects,layers:cfg.result?.layers?.map((l:any)=>({name:l.name,disabledReason:l.disabledReason}))},null,2));const response=await rpc('hooks/list',{cwds:[repo]});if(response.error)throw Error(JSON.stringify(response.error));writeFileSync(join(root,'hooks-loaded.json'),JSON.stringify(response.result,null,2));const hooks=response.result.data.flatMap((row:any)=>row.hooks).filter((h:any)=>h.sourcePath?.replaceAll('\\','/').toLowerCase()===(installed?join(source,'hooks','hooks.json'):join(repo,'.codex','hooks.json')).replaceAll('\\','/').toLowerCase());if(hooks.length!==4)throw Error('Expected four candidate hooks: '+JSON.stringify(response.result));if(installed){if(hooks.some((h:any)=>!h.enabled||h.trustStatus!=='trusted'))throw Error('Installed hooks are not enabled and trusted');return [];}return ['-c','hooks.state={'+hooks.map((h:any)=>JSON.stringify(h.key)+'={trusted_hash='+JSON.stringify(h.currentHash)+'}').join(',')+'}']}finally{clearTimeout(timeout);p.stdin.end();p.kill()}
}
const hotActivation=process.argv.includes('--hot-activation');if(hotActivation&&installed)throw Error('Hot-swap verification is fixture-only');const trust=await hookTrust();let requests=0,issued=hotActivation?-1:0,toolNames:string[]=[],goalResults:any[]=[],beforeActivation:any;
const server=Bun.serve({hostname:'127.0.0.1',port:0,async fetch(req){
 if(req.method!=='POST')return new Response('not found',{status:404})
 const body:any=await req.json();writeFileSync(join(root,'request-shape.json'),JSON.stringify({path:new URL(req.url).pathname,keys:Object.keys(body),model:body.model,tools:body.tools},null,2));const blocked=(body.input??[]).find((x:any)=>x.call_id==='before_activation'&&x.type==='function_call_output');if(hotActivation&&blocked&&!beforeActivation){beforeActivation=blocked;cpSync(join(source,'scripts/hook.js'),join(candidate,'scripts/hook.js'))}goalResults.push(...(body.input??[]).filter((x:any)=>x.type==='function_call_output'&&x.call_id?.startsWith('goal_')));writeFileSync(join(root,'goal-results.json'),JSON.stringify({goalResults,beforeActivation},null,2));requests++;toolNames=(body.tools??[]).map((t:any)=>t.name)
 const id='response_'+requests;let item:any
 if(issued===-1){item={type:'function_call',id:'fc_before',call_id:'before_activation',name:'get_goal',arguments:'{}'};issued++}else if(issued<3){
  const name=['create_goal','get_goal','update_goal'][issued]; const args=issued===0?{objective:'Deterministic ownership hook acceptance'}:issued===2?{status:'complete'}:{}; item={type:'function_call',id:'fc_goal_'+issued,call_id:'goal_'+issued,name,arguments:JSON.stringify(args)};issued++
 }else if(issued===3){
...
try{
 ...
 const dir='.visual-e2e/goal-ownership-'+(installed?'installed':'candidate')+(fixtureSkills?'-fixture-skills':'')+(endedOwner?'-ended-owner':'');mkdirSync(dir,{recursive:true});writeFileSync(join(dir,'actual-tool-stdout.jsonl'),stdout);writeFileSync(join(dir,'actual-tool-stderr.txt'),stderr)
 const status=JSON.parse(readFileSync(join(store.runtime,'coordination',beforeProject+'.json'),'utf8')),worktrees=git('worktree','list','--porcelain'),target=worktrees.split('\n').filter(s=>s.startsWith('worktree ')).map(s=>s.slice(9)).find(s=>s.includes('quest-recovery-'))
