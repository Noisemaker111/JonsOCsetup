...
export async function buildCodexQuest(output:string){
 ...
 const result=await Bun.build({entrypoints:[join(root,'quest/mcp-server.ts'),join(root,'quest/codex/hook.ts'),join(root,'quest/codex/recovery-command.ts'),join(root,'quest/codex/recovery-loader.ts')],target:'bun',outdir:join(output,'scripts'),naming:'[name].js'})
 ...
 const files=['scripts/mcp-server.js','scripts/hook.js','scripts/recovery-command.js','scripts/recovery-loader.js','.codex-plugin/plugin.json','.mcp.json','hooks/hooks.json','skills/quest/SKILL.md']
