...
for(const width of [160,80]){
 ...
 try{
  ...
  await wait('mouse native session',async()=>(await frame()).includes('CHANNEL_REAL_MODEL_READY'));await capture('worker-mouse');result.mouse=true
  await command('/quest-back');await wait('mouse return',async()=>(await frame()).includes('QUEST STEPS'));await capture('returned')
  store.apply(q.id,'stage-state',{stageID:'navigate',status:'done',evidence:'Actual installed keyboard and mouse opened the native readiness transcript and returned'},'verification')
