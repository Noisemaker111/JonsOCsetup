...
const fixtures=[1,2].map(number=>{const directory=join(fixture,'project-'+number);mkdirSync(directory);writeFileSync(join(directory,'AGENTS.md'),'This is installed verification project '+number+'. Read only this instruction file and report the project number honestly. Do not edit files or run shell commands.\n');report.projects.push(directory);return {directory,expectedProject:projectIdentity(directory)}})
{
 ...
 try{
  ...
  for(number=1;number<=2;number++){
   ...
   await command(`Select project ${directory.replaceAll('\\','/')} with project_select action select. Create two independent Quests titled Installed single giver project ${number} and Installed concurrent sibling ${number}. For both use description: Read only AGENTS.md in the assigned directory, save INSTALLED_QUEST_WORKER_VERIFIED plus the actual project number and one instruction in assigned step note with quest update.steps state done, then quest get to verify. Create one step id inspect, title Read AGENTS.md and save verified result. Run each newly created Quest exactly once immediately, without waiting for either worker to finish, with run {stepIDs:["inspect"],model:"${workerModel}",readOnly:true}. Use Code Mode Promise.all for the two independent run calls. Do not retry, substitute models, create another giver, or release account holds. Report the admission and finish without polling. When an automatic Quest worker update arrives, get the saved Quest, report its actual saved finding and outcome, and finish without dispatch.`)
