...
Install the command from the durable source checkout after integration: run `npm link` in `C:/Users/Jk101/Projects/JonsOCsetup`. Do not link an implementation worktree. `quest --help` lists generated operations; `quest create --help` describes nested fields and `quest create --help --json` prints the contract. Pipe a JSON object to `quest create --input-json` for multiline descriptions and steps.
