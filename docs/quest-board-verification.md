...
| `quest/worker-observation.mjs`, `worker-inspection.ts`, `service.ts` | Separate live observations from ledger state; bounded inspections; reconcile missed terminal events through QuestTracker; no completion from silence or an old message. `quests.get` with `inspect.section: runs` exposes observations. |
