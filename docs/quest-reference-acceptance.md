...
The first installed concurrency candidate admitted four real Sol/xhigh workers across two overlapping pairs, with every saved step done, terminal outcome succeeded and reservation settled. Its final older-Quest composer return failed: the board selected the newest Quest because an initial empty records effect erased initialQuestID before loading. A real-render regression failed at width 80 before the fix and passed at 80/160 afterward. The board now waits for the first successful record load before replacing a missing selection. Native navigation acceptance now explicitly selects and verifies the intended Quest title, rather than accepting any worker marker. The failed candidate report is .visual-e2e/installed-single-giver-1789007028624/report.json; it is not accepted for activation.

Final concurrency-only full suite: 964 passed, one existing skip, zero failures, 7,537 assertions, 336.04 seconds. Focused return/render/navigation checks after the initialization fix: 34 passed, zero failed, 154 assertions.
