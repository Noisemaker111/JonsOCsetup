...
- Verify every change by using the installed OpenCode2 app and inspecting the saved result after reopening. Keep a small core suite that directly exercises consequential production invariants; remove bloated feature/scenario suites and mirrored implementations. OpenCode, Codex and Claude must drive the product through the same controls the user uses. Build/type checks may supplement real app use. Report observed failures honestly.
