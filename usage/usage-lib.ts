...
import { getAccountUsage, readAccountUsage, routeAccountCapacity, accountSummaryLine, type AccountSnapshot } from "./account-api"
import { projectAccountUsage } from "./account-projection"
...
export const USAGE_CACHE_FILE = SHARED_USAGE_CACHE_FILE
...
export type UsageCache = { updated: string; sources: UsageSource[]; accounts?: AccountSnapshot }
...
export type CapacitySnapshot = { updated: string; stale: boolean; providers: Record<string, CapacityEntry>; accounts?: AccountSnapshot }
export { routeAccountCapacity, getAccountUsage }
...
export function usageCache(): UsageCache | undefined {
  const legacy = (readJson(USAGE_CACHE_FILE) ?? readJson(LEGACY_USAGE_CACHE_FILE)) as UsageCache | undefined
  return projectAccountUsage(legacy, readAccountUsage())
}
...
export function usageWindowResetAt(cache: UsageCache | undefined, win: UsageWindow | undefined): number | undefined {
  if (win?.resetAt && Number.isFinite(Date.parse(win.resetAt))) return Date.parse(win.resetAt)
...
export function capacitySnapshot(cache: UsageCache | undefined, now = Date.now()): CapacitySnapshot {
  ...
  for (const source of cache?.sources ?? []) {
    const sourceAge = usageAgeMs(source.observedAt ? { updated: source.observedAt, sources: [] } : cache, now)
    const sourceStale = sourceAge < 0 || sourceAge >= (source.accountID ? USAGE_STALE_MS : 15 * 60_000)
    const scoped = source.windows?.filter(w => !w.scope || w.scope === "shared")
    const cappedWindows = scoped?.filter((win) => windowCapped(win) && (usageWindowResetAt(cache, win) ?? Infinity) > now) ?? []
    const hadExpiredCap = scoped?.some((win) => windowCapped(win) && (usageWindowResetAt(cache, win) ?? Infinity) <= now) === true
    const capped = cappedWindows.length > 0 || Boolean(source.apiCapHit && !hadExpiredCap)
    const official = scoped?.some((win) =>
    ...
    const resetAt = cappedWindows
      ...
      .sort((a, b) => a - b)[0]
    providers[source.id] = {
      state: sourceStale || hadExpiredCap ? "unknown" : capped ? "exhausted" : source.probe === "ok" && official ? "available" : "unknown",
      ...(capped && resetAt != null ? { resetAt } : {}),
      authenticated: source.probe === "ok" ? true : source.probe === "err" ? false : undefined,
    }
  }
  return { updated: cache?.updated ?? "", stale, providers, ...(cache?.accounts ? { accounts: cache.accounts } : {}) }
}
...
export async function ensureUsageCache(force = false): Promise<{
  ...
  // Account refresh is shared with the collector and TUI; no additional login or per-consumer probes.
  let accountFailed = false
  try { await getAccountUsage({ refresh: force }) } catch { accountFailed = true }
  const existing = usageCache()
  const local = readJson(USAGE_CACHE_FILE) as UsageCache | undefined
  if (!force && usageAgeMs(local) < USAGE_STALE_MS) {
    return { cache: existing, collectFailed: accountFailed, stale: false }
  }
  let collectFailed = accountFailed
  try {
    const ok = await awaitCollect(startUsageCollector({ force }), USAGE_COLLECT_AWAIT_MS)
    collectFailed = collectFailed || !ok
  } catch (error) {
    collectFailed = true
...
export function quotaSummaryLine(cache: UsageCache | undefined): string {
  if (cache?.accounts?.accounts.length) return accountSummaryLine(cache.accounts)
  const age = usageAgeMs(cache)
