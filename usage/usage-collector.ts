// Account observations plus a compatibility projection of shared request telemetry.
// API-equivalent values never become subscription percentages or actual charges.
import { observedWindowRate, aggregateTelemetry, type RequestRecord } from "./telemetry"
import { readRequests } from "./telemetry-store"
import { getUsageStatus } from "./status-api"
import { getAccountUsage } from "./account-api"
import { SHARED_USAGE_CACHE_FILE, LEGACY_USAGE_CACHE_FILE } from "./usage-paths"
...
const CACHE_FILE = SHARED_USAGE_CACHE_FILE
...
export function predictWindowExhaustion(opts: {
  ...
  const observed=observedWindowRate({before:opts.priorPct!,after:opts.pct!,beforeAt:opts.priorAt!,afterAt:opts.now,resetAt:opts.resetAt!,beforeResetAt:opts.priorResetAt!})
  if(!observed||remaining===null)return {state:"unknown",confidence:"low",horizonSeconds:null,assumesResetAt:resetAt,basis:"insufficient-data"}
  const horizonSeconds=Math.round(observed.secondsToExhaustion),remainingWindow=(opts.resetAt!-opts.now)/1000
  const state=horizonSeconds>remainingWindow?"unlikely":horizonSeconds<=3600?"likely":"at-risk"
  return {state,confidence:"low",horizonSeconds,assumesResetAt:resetAt,basis:"provider-rate"}
}
...
async function providerProbeHeaders(source: string, url: string): Promise<Record<string, string> | undefined> {
  if (source === "cursor" && new URL(url).origin === "https://api.cursor.com" && process.env.CURSOR_API_KEY) return { Authorization: "Bearer " + process.env.CURSOR_API_KEY }
  return undefined
}
...
export function derivePctFromCap(sourceId: string): boolean {
  // Subscription dollars are not a quota denominator. Only metered (pay-per-token)
  // plans may derive a percentage from a configured local spend cap.
  return KIND[sourceId] === "metered"
}
...
export function finalizeWindow(
  ...
  const prediction = predictWindowExhaustion({
    ...
    resetAt,
    windowMs,
    priorPct: win?.priorPct,
    priorResetAt: win?.priorResetAt,
...
/** Compatibility cache projection; all token and price arithmetic lives in telemetry.ts. */
export function aggregateRequestHistory(records: RequestRecord[], now = Date.now()) {
  const bySource = new Map<string, SourceAgg>()
  const summarize = (rows:RequestRecord[]) => aggregateTelemetry(rows)
  for (const providerID of new Set(records.map(r=>r.route.providerID))) {
    const rows=records.filter(r=>r.route.providerID===providerID), sourceID=PROVIDER_TO_SOURCE[providerID]??providerID
    const agg=ensureSource(bySource,sourceID)
    for(const modelID of new Set(rows.map(r=>r.route.modelID))){
      const summary=summarize(rows.filter(r=>r.route.modelID===modelID)), value=summary.apiEquivalent.USD
      agg.windows.models.set(providerID+"/"+modelID,{providerID,modelID,messages:summary.requests,tokens:summary.tokens.knownTotals as TokenBucket,cost:value?.knownValue??0,dbCost:summary.actualCharges.USD??0,priceSource:"shared request price snapshots; API-equivalent known subtotal, "+Object.values(summary.apiEquivalent).reduce((n,v)=>n+v.unavailableRequests,0)+" requests unvalued"})
    }
    for(const window of WINDOWS){
      const selected=rows.filter(r=>r.startedAt>=now-window.ms),summary=summarize(selected),prior=agg.windows.windows[window.label]
      agg.windows.windows[window.label]={usedTokens:(prior?.usedTokens??0)+Object.values(summary.tokens.knownTotals).reduce((n,v)=>n+(v??0),0),used:(prior?.used??0)+(summary.actualCharges.USD??0),dbCost:(prior?.dbCost??0)+(summary.actualCharges.USD??0),oldestTs:Math.min(prior?.oldestTs??now,...selected.map(r=>r.startedAt))}
    }
  }
  return bySource
}
  ...
  for (const source of previous.sources) {
    ...
    for (const prior of source.windows ?? []) {
      ...
      if (current && used != null && used >= 0) {
        current.priorPct = finiteOrNull(prior.pct) ?? undefined
        current.priorResetAt = typeof prior.resetsInSeconds === "number" ? priorAt + prior.resetsInSeconds*1000 : undefined
...
export function buildCache(
  ...
  for (const agg of bySource.values()) {
    ...
    const source: any = {
      id: agg.id,
      kind: agg.kind,
      source: "shared-request-telemetry",
      windows,
  ...
  // Sources not in `order` (unknown-shell claude-code/codex/grok-build, or a
  // configured-but-unlisted source like openrouter) must sort AFTER the known
  // ones, never before: indexOf returns -1 for them, and -1 sorts first, which
  // pushed empty shells ahead of every working block in the dialog. Within that
  // remainder, sources with real usage or a live probe (e.g. openrouter) still
  // outrank the truly empty config shells (claude-code/codex/grok-build) — an
  // unranked source that actually has data must never land behind an unranked
  // source that never will.
  const hasRealData = (s: any) =>
    (s.windows?.some((w: any) => (w.usedTokens ?? 0) > 0 || (w.used ?? 0) > 0) ?? false) ||
    (s.probe != null && s.probe !== "none")
  const rank = (s: any) => {
    const i = order.indexOf(s.id)
    if (i !== -1) return i
    return hasRealData(s) ? order.length : order.length + 1
  }
  kept.sort((a, b) => rank(a) - rank(b) || a.id.localeCompare(b.id))
  return {
...
async function collectMain() {
  ...
  const now = new Date()
  const previous = readUsageCache(CACHE_FILE, now.getTime()) ?? readUsageCache(LEGACY_USAGE_CACHE_FILE, now.getTime())
  const plans = readJsonOr(PLANS_FILE, { plans: {} })
  const stored = readRequests()
  const records = stored.records.filter(r=>r.startedAt>=now.getTime()-WINDOWS[2].ms)
  for(const diagnostic of stored.diagnostics)log(diagnostic)
  log("Shared request records: "+records.length)

  const [localProbes, accountSnapshot] = await Promise.all([runProbes(), getAccountUsage()])
  const goAccount = accountSnapshot.accounts.find(a => a.provider === "opencode-go")
  const goProbe: ProbeResult = goAccount?.observedAt && !goAccount.error ? {
    source: "opencode-go", status: "ok", detail: "shared account API",
    usage: Object.fromEntries(goAccount.windows.map(w => [w.id, { percent: w.usedPercent, status: w.state === "exhausted" ? "rate-limited" : "ok", resetsAt: w.resetAt }])),
  } : { source: "opencode-go", status: "error", detail: "shared account API unavailable" }
  const probes = [...localProbes, goProbe]
  for (const p of probes) log(`probe ${p.source}: ${p.status} (${p.detail})`)

  const bySource = aggregateRequestHistory(records, now.getTime())
  ...
  const apiCapHit = goMerge.apiCapHit || unavailableGo
