...
One `execute` costs a model round trip, measured at 5.0s median between consecutive tool calls in your own sessions, so what a turn costs is how many times you call `execute`, not the work inside one. Three quarters of past executes ran a single call. Put everything the turn already knows it needs into one program: gather with `Promise.all`, branch on the results in the same code, and return what you actually need to read. Split into a second execute only when the next call depends on something you could not know before reading the first result -- never to do one more thing you could have listed up front.
