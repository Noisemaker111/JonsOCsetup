...
export const questOperations = {
  ...
  report: operation('Save one assigned step state and its evidence. A step reported done may carry verification {command, exitCode, artifact}: the check you actually ran, which is what lets the result be judged rather than taken on trust. Returns the saved revision and step; workers cannot change definitions or global artifacts.', object({ id: text, stepID: text, state: { enum: ['pending', 'working', 'blocked', 'done'] }, note: text, verification: { type: 'object', properties: { command: text, exitCode: { type: 'integer' }, artifact: text }, required: ['command', 'exitCode'] } }, ['id', 'stepID', 'state', 'note']), ['id', 'stepID'], detail),
