...
export type QuestSession = {
  ...
  permissionDecisions?: Array<{requestID:string;reply:"once"|"reject";state:"sending"|"acknowledged"|"unknown";actor:"user"|"giver";reason:string;at:string}>
}
...
export type QuestStage = {
  id: string; title: string; commandID?: string; note?: string; detail?: string; status: QuestStageStatus; needs: string[]; todos: QuestTodo[]
...
export type QuestArtifactRef = {
  name: string
  /** Repo-relative stored capture, e.g. `.opencode/quests-assets/<questID>/before-board.png`. */
  path?: string
  uri?: string
  /** Chain label: "before", "after", or "revision N". */
  label?: string
  revision?: number
  at?: string
  digest?: string
...
export type Quest = {
  contractVersion?: 2; description?: string; reward?: string
  project?: { id: string; root: string }
  archive?: { at: string; reason?: string; accepted: boolean }
...
export const DEFAULT_COMPLETION_POLICY: CompletionPolicy = {
  requireSessions: true, requireCommits: false, requireTests: true, requireReview: false,
  requireArtifacts: false, requirePublish: false, requireWorktreeEquality: false,
}
