...
import {readQuestActivity,questActivity,ownedRuns} from './activity'
...
export function workSupplyTool(store:QuestStore,host:QuestHost){return {name:'quest_work_supply',description:'Bounded inventory of existing pending Quest steps in this project or explicitly across projects. Lists dependency-ready work, active workers and blocking conditions for pacing. Does not invent tasks, dispatch, or count queued work as workers.',output,input:{type:'object',additionalProperties:false,properties:{limit:{type:'integer',minimum:1,maximum:50},allProjects:{type:'boolean'}}},execute:async(input:{limit?:number;allProjects?:boolean},ctx:any)=>{const {context}=await caller(store,host,ctx);const records=readAllQuests(store.projectRoot),quests=records.flatMap(r=>r.quest&&(input.allProjects||r.quest.project?.id===context.project.id)&&r.quest.state!=='Archived'?[r.quest]:[]);const snapshot=await readQuestActivity(host,quests);let active=0,ready=0,running=0;const items=quests.map(q=>{const workers=ownedRuns(q),activity=questActivity(q,snapshot);active+=workers.length;running+=activity.running;const steps=q.stages.filter(s=>s.status==='pending'&&s.needs.every(id=>q.stages.find(d=>d.id===id)?.status==='done')&&!workers.some(w=>w.deliverables.includes(s.id)));ready+=steps.length;return {questID:q.id,title:q.title,project:q.project,readySteps:steps.map(s=>({id:s.id,title:s.title,needs:s.needs})),active:activity.running,unknown:activity.unconfirmed,assigned:activity.assigned,checkedAt:activity.checkedAt}}).filter(q=>q.readySteps.length||q.assigned);return result({projectID:context.project.id,readySteps:ready,activeRuns:running,unconfirmedRuns:active-running,activeOrUncertainRuns:active,items:items.slice(0,input.limit??20),truncated:items.length>(input.limit??20),diagnostics:records.flatMap(r=>r.quest?[]:r.errors),note:'Ready dependency state is not dispatch authorization or capacity; exact route, workspace and pacing checks apply at admission. Cross-project work requires its own verified destination-bound giver.'})}}}
