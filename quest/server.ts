 ...
 */
export function installQuestEvents(ctx: { event?: { subscribe?: Function }; session?: any; permission?: any; catalog?: any }, quests: QuestTracker) {
  ...
  if (!owner) return
  if(ctx.session)registerHostObservation(ctx.session,ctx.permission,ctx.catalog)
