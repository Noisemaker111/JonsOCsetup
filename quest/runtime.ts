...
export function startQuestRun(store:QuestStore,host:QuestHost,options:{policyFile:string;settingsFile?:string;reserve?:typeof reserveDispatch;beforePrompt?:typeof assertBurnLaunchAllowed}):StartRun {
 ...
 return async input=>{
  ...
  try{
   ...
   if(selected.decision)store.apply(input.quest.id,"session-state",{callID:input.runID,state:"executing",routingNote:selected.decision.summary,evidence:selected.decision.summary},"quest:routing")
