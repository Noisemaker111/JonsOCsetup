...
/** Every step needs one passed command proof; `step done` records it from the report. Visible work may add run/judgment proofs. */
export function requiredProofKinds(_stage: Pick<QuestStage, "claim">): QuestProof["kind"][] {
  return ["command"]
}
export function isVisibleStage(stage: Pick<QuestStage, "claim">): boolean {
  return stage.claim.include.some((path) => VISIBLE_PATH.test(path.replaceAll("\\", "/")))
}
