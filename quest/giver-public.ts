...
import {ensureUserGiver,selectUserGiverProject,readGiverProjectSelection,userGiverID} from './user-giver'
...
export function selectGiverProject(sessionID:string,targets:{directory:string}[],revision:number,options:Parameters<typeof selectUserGiverProject>[4]={}){return selectUserGiverProject(new QuestStore(questRoot()),sessionID,targets,revision,options)}
export function giverProjectSelection(sessionID:string){return readGiverProjectSelection(new QuestStore(questRoot()),sessionID)}
