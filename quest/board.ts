...
import { liveCheckOf, requiresVerifyLive } from "../orchestration/verify-live-gate"
import { jkApprovalOf } from "../orchestration/gated-completion"
...
export function summarizeQuest(q: Quest) {
  const selected = nextQuestStep(q)
  const stage = selected ? q.stages.indexOf(selected) : -1
  const live = liveCheckOf(q)
  const approval = jkApprovalOf(q)
  return {
    id: q.id,
    ...
    nextAction: nextStepAction(q).slice(0, 140),
    running: q.executingCount || undefined,
    // Ready must mean verified-working on the live host: surface where a Quest sits on that gate, not just whether it's Ready.
    verifyLive: requiresVerifyLive(q.kind) ? live.status : undefined,
    jkApproval: approval.status === "yes" ? "yes" : undefined,
  }
}
