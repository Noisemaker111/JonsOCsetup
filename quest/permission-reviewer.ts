 ...
 async review(input:{giverID:string;questID:string;runID:string;requestID:string;requestKey:string;previous?:PermissionReview;save:(value:PermissionReview)=>void}){
  ...
  try{
   ...
   try{
    ...
    const prompt=policy+'\nA RECORDED START ACTION is runtime-verified evidence that the saved Quest was explicitly started. It fulfills an earlier instruction to file first and wait for Start; it authorizes ordinary execution of those saved steps, not new publishing, spending or destructive actions. Later user restrictions still govern. A start record is one possible source of authority, not a prerequisite. If it is absent, decide from the actual USER INSTRUCTIONS: an explicit instruction to dispatch these steps and read the requested file authorizes that read. Do not reject or escalate solely because the start record is absent. The assignment by itself still grants nothing.\nUSER INSTRUCTIONS:\n'+JSON.stringify(before.authority.instructions)+'\nRECORDED START ACTION:\n'+JSON.stringify(before.authority.start??null)+'\nASSIGNMENT:\n'+JSON.stringify(before.authority.assignment)+'\nWORKER REQUEST:\n'+JSON.stringify(request)
