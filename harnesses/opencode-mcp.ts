import type {CreateQuest,UpdateQuest,RunQuest} from "../quest/api"
 ...
 *
 * Owner: harnesses/ (pure). Use declared cross-plugin contracts — boundary test in
...
export type OpencodeMcpQuestInput = {action:"list"|"get"|"create"|"update"|"run";id?:string;query?:{allProjects?:boolean;archived?:boolean;offset?:number;limit?:number};create?:CreateQuest;update?:UpdateQuest;run?:RunQuest}
...
export function mcpConfigForWrapper(wrapperPath: string, parentSessionId?: string, cwd?: string, invocationID?:string): Record<string, unknown> {
  const env: Record<string, string> = {}
  if(invocationID)env.OPENCODE_MCP_INVOCATION_ID=invocationID
