...
import { recordHang } from "../papercut/server"
...
/**
 * The live line for a harness run: `(quest title, claude-code/alias)`. The
 * quest title comes from the canonical ledger (quest/store.ts + quest/root.ts
 * are declared public surfaces); the CLI takes no reasoning effort and has no
 * fast variant, so none is named. Anything unreadable — no quest, "unbound",
 * a broken ledger — falls back to today's `provider/model - task` title
 * instead of failing the run.
 */
export function questFaceTitle(questID: string | undefined, identity: { providerID: string; modelID: string; task: string }): string {
  if (questID && questID !== "unbound") {
    try {
      const title = new QuestStore(questRoot()).read(questID)?.title?.trim()
      if (title) return subagentChipLabel({ title }, { providerID: identity.providerID, modelID: identity.modelID })
    } catch { /* fall through to the model-task title */ }
  }
  return canonicalWorkerTitle(identity)
}
...
export async function runClaudeCodeTask(input: ClaudeCodeTaskInput, context: ClaudeCodeTaskContext = {}) {
  ...
  // The live line, not "model - task": the spawn ledger description and the
  // result title/label are this run's faces, and they must read exactly like
  // the native background chip. The CLI takes no reasoning effort, so none is
  // named; a run with no readable Quest keeps today's model-task title.
  const face = questFaceTitle(questID, identity)
  appendLedger({ kind: "spawn", parentID, callID, agent: "claude-code", agentRole: identity.agentRole, providerID: identity.providerID, modelID: identity.modelID, runtime: identity.runtime, runID, task: identity.task, description: face, questID })
  ...
  try {
    ...
    return { content: `Claude Code (Harness) completed:\n${cli.text}`, metadata: { runtimeSessionId: cli.sessionId, sessionId: cli.sessionId, runID, runtime: "claude-code", agentRole: "claude-code", providerID: "claude-code", modelID: alias, parentID, task: identity.task, title: face, label: face } }
  } catch (error) {
