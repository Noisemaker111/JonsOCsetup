...
const TOOLS = [
  ...
  {
    name: "models",
    description: "List exact models known to this host. Route selection belongs to quest run and configured policy.",
    inputSchema: {
      type: "object",
  ...
  {name:"quest",description:"Durable project work: list, get, create, update, run. Uses the same typed API as native OpenCode. Steps drive progress; failures remain errors.",inputSchema:QUEST_TOOL_INPUT,outputSchema:QUEST_TOOL_OUTPUT},
]
...
// Lane default mirrors opencode.jsonc's base settings.reasoningEffort per
// model — a labeling mirror, not a runtime setting.
const CHIP_REASONING_DEFAULT = {
...
/**
 * The one live line every dispatch surface carries: `(quest title, model,
 * reasoning[, fast])`, unknown parts omitted, never placeholder text. Same
 * format as subagentChipLabel in orchestration/dispatch.ts (kept in sync by
 * test: opencode-mcp-gateway asserts both shapes agree). Harness sessions
 * have no host background chip, so this goes in the POST /session title —
 * the session list then shows the same face the native chip shows.
 */
export function chipLabel(questTitle, model) {
  const name = model && model.providerID && model.id ? `${model.providerID}/${model.id}` : undefined
  let reasoning
  let fast = false
  if (model) {
    const variant = String(model.variant ?? "").toLowerCase()
    const hint = variant.match(/^(none|low|medium|high|xhigh|max)(-fast)?$/)
    // A `-fast` model id is itself a fast variant even when the explicit
    // hint names no -fast suffix — same rule as reasoningEffortFor.
    fast = Boolean(hint?.[2]) || /-fast$/i.test(String(model.id ?? ""))
    reasoning = hint?.[1] ?? CHIP_REASONING_DEFAULT[`${model.providerID}/${model.id}`.toLowerCase()]
  }
  const parts = [questTitle, name, reasoning].filter(Boolean)
...
async function startDeskSession(input) {
  ...
  if (cwd && session?.location?.directory !== cwd) {
    ...
    const bound = await call("GET", `/session/${sessionID}`)
    const actual = bound?.location?.directory
    const pathKey = (value) => process.platform === "win32" ? resolve(value).toLowerCase() : resolve(value)
    if (typeof actual !== "string" || pathKey(actual) !== pathKey(cwd)) throw new Error(`Workspace binding failed for ${sessionID}; prompt was not sent`)
  }
...
async function startAgent(input) {
  ...
  let title
  let sessionID
  // The live line, not "model - task": the session title is the harness
  // session's face in the host session list, and it must read exactly like
  // the native background chip (quest title, model, reasoning, fast).
  const questTitle = questField(questSource(cwd, questID), "title", "Untitled Quest")
  if (requestedSessionID) {
    ...
    title = chipLabel(questTitle, model)
  } else {
...
async function pickModel(_input) {
 const models=await api()("GET","/model"),list=Array.isArray(models?.data)?models.data:Array.isArray(models)?models:[]
 const structuredContent={models:list.map(model=>model.providerID+"/"+(model.modelID??model.id)).sort(),selection:"Use quest run with user-configured routing policy."}
 return {structuredContent,content:[{type:"text",text:JSON.stringify(structuredContent)}]}
}

// --- quest ---------------------------------------------------------------
// quest-cli.ts is the same canonical QuestStore/agent-api the native `quest`
// tool uses (plugins-active/quests.ts); shelling out to it keeps this
// self-contained script from statically importing the TS quest subsystem
// while still writing through the one locked, event-sourced ledger writer
// instead of re-implementing step/evidence mutation here.
const QUEST_CLI = join(dirname(SELF), "..", "quest", "quest-cli.ts")

function runQuestCli(args, cwd, input, requestID) {
  return new Promise((resolvePromise, reject) => {
    const child = spawn("bun", [QUEST_CLI, ...args], {
      cwd: cwd || process.env.OPENCODE_CWD || process.cwd(),
      env: { ...process.env, OPENCODE_QUEST_ROOT: process.env.OPENCODE_QUEST_ROOT || homedir(),...(requestID?{OPENCODE_QUEST_REQUEST_ID:requestID}:{}) },
...
async function questAction(input, requestID) {
  if(["list","get","create","update","run"].includes(input.action)){const {code,stdout,stderr}=await runQuestCli(["api"],undefined,input,requestID);if(code!==0)throw new Error(stderr.trim()||"Quest operation failed");const structuredContent=JSON.parse(stdout);return {structuredContent,content:[{type:"text",text:JSON.stringify(structuredContent)}]}}
  const action = String(input.action ?? "").trim()
  const id = String(input.id ?? "").trim()
  if (!id) throw new Error("quest: id is required")
  if (action === "get") {
    const { code, stdout, stderr } = await runQuestCli(["get", id])
    if (code !== 0) throw new Error(stderr.trim() || "quest get failed")
    return { content: [{ type: "text", text: stdout.trim() || "null" }] }
  }
  if (action === "step") {
    const stepID = String(input.stepID ?? "").trim()
    if (!stepID) throw new Error("quest: stepID is required for action=step")
    if(!["pending","working","blocked","done"].includes(input.state))throw new Error("quest: explicit step state is required")
    const state = input.state
    const { code, stdout, stderr } = await runQuestCli(["step", id, stepID, state, String(input.value ?? "")])
    if (code !== 0) throw new Error(stderr.trim() || "quest step failed")
    return { content: [{ type: "text", text: stdout.trim() }] }
  }
  if (action === "evidence") {
    const kind = String(input.kind ?? "").trim()
    if (!kind) throw new Error("quest: kind is required for action=evidence")
    const { code, stdout, stderr } = await runQuestCli(["evidence", id, kind, String(input.value ?? "")])
    if (code !== 0) throw new Error(stderr.trim() || "quest evidence failed")
    return { content: [{ type: "text", text: stdout.trim() }] }
  }
  if (action === "report") {
    const text = String(input.value ?? "").trim()
    if (!text) throw new Error("quest: value (the STEP/TESTS report text) is required for action=report")
    const { code, stdout, stderr } = await runQuestCli(["report", id, text])
    if (code !== 0) throw new Error(stderr.trim() || "quest report failed")
    return { content: [{ type: "text", text: stdout.trim() }] }
  }
  throw new Error(`quest: unsupported action ${action}`)
}
...
rl.on("line", async (line) => {
  ...
  try {
    if (method === "initialize") {
      ...
      else if (name === "models" || name === "pick_model") result = await pickModel(args)
      ...
      else if (name === "quest") result = await questAction(args,"mcp:"+(process.env.OPENCODE_MCP_INVOCATION_ID??"session")+":"+String(id))
