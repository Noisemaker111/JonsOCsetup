...
- **Slot names are dotted** (host renderer paths): `app` · `home.footer` · `prompt.footer` · `prompt.footer.file` · `prompt.footer.status` · `session.composer.top` · `sidebar.content` · `sidebar.footer`. Underscored `app_bottom` / `sidebar_content` never matches. Inspect the current installed renderer when the host changes.
  ...
  - `prompt.footer` shares a single row with native controls and the directory. Keep cumulative usage and multi-row Quest status in `session.composer.top`; use `home.footer` for the home route. Do not append long status blocks into the native row.
...
6. Keep Quest status in its dedicated composer rows and retain the sidebar entry. Verify native commands, directory, usage and Quest click targets at 80 columns and wider in the installed host.
