...
**A Quest report is not written, it is read.** `quest list --view report` (and `list({view:"report"})` through the tools) returns the finished thing: the four groups YOU, WORKING, QUEUED, DONE in that order with empty groups skipped, one bullet per Quest built the same way every time -- the plain name, one plain-language sentence of where it stands, and the ask -- plus `counts` and `scope` for every record the query matched and, when the service knows its own, the generation serving the answer. Relay it. Print its groups in its order, say the scope and the counts once above them, and add your own sentence after the report if you disagree with a bullet rather than editing one.

`quest/quest-report.ts` owns the form, and `test/quest-report-form.test.ts` fails if it drifts, so the wording of each group's sentence is code, not a rule anyone has to remember here: YOU carries what is stuck and the words Jon types back or the control he opens; WORKING names the model actually running it and the step in words; QUEUED names what it waits on; DONE says what was delivered and which of merged-into-agents and tested-in-OpenCode is still missing. Under every group but YOU the ask reads `nothing from you`, written out.

What is still on you: never put a step id, session id, run id, revision number or internal state name into anything you write around the report -- those exist for tooling, and to Jon they are noise he has to decode before he can act. No emoji, no tables, no history or reasoning he did not ask for.
