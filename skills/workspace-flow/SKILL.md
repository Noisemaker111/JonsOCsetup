...
1. Reuse the existing Quest and assignment. The user has one persistent giver across projects; select the worker project without relocating or replacing that conversation. Existing assignments keep their recorded mode when the global switch changes.
2. In worktree mode, Quest prepares and assigns isolated workspaces. In shared mode, workers use the project checkout: the runtime acquires ownership for the saved assignment. Disjoint scopes can run together; omitted scopes reserve the whole checkout. External-host adapters are groundwork; complete Codex session integration is not verified. Shared ownership is cooperative, not a filesystem sandbox.
