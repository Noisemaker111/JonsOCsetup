...
import { isClaudeCodeModel, isForbiddenXai, overlayProviderLane, splitProviderModel } from "./model-catalog"
...
export function sourceCapHit(sourceId: string, cache: UsageCache | undefined = usageCache()): Cap {
  ...
  for (const win of src.windows ?? []) {
    if (win.scope && win.scope !== "shared") continue
...
function hasAuthoritativeQuota(providerID: string, cache: UsageCache | undefined): boolean {
  ...
  const age = usageAgeMs(source?.observedAt ? { updated: source.observedAt, sources: [] } : cache)
  if (!Number.isFinite(age) || age < 0 || age >= USAGE_STALE_MS) return false
  if (!source || !Array.isArray(source.windows)) return false
  const labels = source.accountID ? source.windows.filter(w => w.scope === "shared").map(w => w.label) : CAP_WINDOW_LABELS
  ...
  for (const label of labels) {
    ...
    if (source.accountID && (source.probe !== "ok" || win.status === "unknown" || win.provenance !== "provider-observed")) return false
...
function applySpawnTarget(input: Record<string, unknown>, providerID: string, modelID: string): string {
  ...
  assertConfiguredModel({ providerID, modelID })
  input.agent = agent
  input.model = model
...
const FALLBACK_FREE = { providerID: "opencode", modelID: "muse-spark-1.3-contributor-free" }
...
/** Next healthy spawn target after a quota/limit rewrite. Never xai. Last resort is Muse-free, never a paid API. */
export function nextHealthyFallback(from: string, cache?: UsageCache): { providerID: string; modelID: string } {
  const order =
    from === "openai" ? [FALLBACK_GROK, FALLBACK_FREE]
    : from === "grok-sub" ? [FALLBACK_LUNA, FALLBACK_FREE]
    : from === "openrouter" ? [FALLBACK_GROK, FALLBACK_LUNA]
    : [FALLBACK_GROK, FALLBACK_LUNA, FALLBACK_FREE]
  ...
  return candidates[0] ?? FALLBACK_FREE
}
...
function overlayLane(profile: Profile, overlay: Lane | undefined): Profile {
  ...
  return {
    ...profile,
    lane: overlay,
    best: `Unavailable under subscription-only policy: ${profile.name} API route`,
...
function routingCard(favs: Fav[]) {
  ...
  const lines = [
    ...
    "Go-quota/Sub/Free lanes are cheap for you even if the list price looks big (Luna/Sol ride OpenAI subs; Grok 4.6 rides SuperGrok via cliproxyapi/grok-4.6 (CLIProxyAPI) ONLY - xai/* bills API per token). Metered = pay-per-token and is disabled, including named requests.",
...
export function pickModel(task: string, favs: Fav[], usage?: UsageCache) {
  ...
  const named = matches.find((fav) => text.includes(`${fav.providerID}/${fav.modelID}`.toLowerCase())) ?? matches[0]
  if (named) {
    assertConfiguredModel(named)
  ...
  const pool = (capHit ? safeFavs.filter((fav) => inferProfile(fav, profiles).lane !== QUOTA_LANE.lane) : safeFavs)
    ...
    // Astra is plan-only escalation, stricter than Sol: never an automatic pool pick, not even for
    // planning-shaped tasks. An explicit name takes the named path above; the automatic pool must
    // not burn the frontier sub on grunt work. (Astra carries no price record, so without this it
    // also dodges the cost penalty every priced model pays and wins heavy tasks by default.)
    .filter((fav) => inferProfile(fav, profiles).id !== "gpt-6-astra")
  if (capHit && pool.length === 0) {
    const grok = safeFavs.find((fav) => (fav.providerID === "cliproxyapi" || fav.providerID === "grok-sub") && /grok/i.test(fav.modelID)) ?? FALLBACK_GROK
...
export function capResetAt(providerID: string, cache?: UsageCache): string | undefined {
  const capped = cache?.sources?.find(s => s.id === providerID)?.windows?.filter(w => (!w.scope || w.scope === "shared") && windowCapped(w)) ?? []
  const resets = capped.map(w => usageWindowResetAt(cache, w))
  if (!resets.length || resets.some(t => t == null)) return undefined
