...
The fenced [`benchmark-routing`](#machine-readable-routing-table) block is a published-prior input parsed by `benchmark-table.ts`. It is one input to the [user-configured planner](../docs/account-aware-routing.md), alongside measured outcomes, account availability, task constraints, prices, and access policy. These dated scores do not prescribe a default model or reasoning level.
...
| GPT-5.6 Terra | **69.6%** max; 60.2% xhigh; 53.8% high | **88.0%** max (AA) | — | **63.4%** *vendor* (OpenAI) | **56.6** max (AA scrape) | At the recorded max effort, DeepSWE ≈ Fable 5 and Luna; at high, **53.8%** vs Sol **69.4%** / Opus **72.8%**. OpenAI vendor TB 2.1 = 87.4%. |
...
## Selection constraints and source check — 2026-09-12

Jon prohibits Luna medium across all roles, explicit selectors, fallback, resume and manual composer requests. `access-policy.json` owns the rule; neither a benchmark nor an unavailable endpoint can weaken it. Luna requires an explicit permitted effort. Max is a candidate only where the exact transport supports it; do not alias it or suggest a lower-effort workaround. Astra medium remains independently authorized. Automatic dispatch has no unconditional primary model.

Re-fetched the [DeepSWE primary JSON](https://deepswe.datacurve.ai/artifacts/v1.1/leaderboard-live.json): generated September 3, 113 tasks, mini-swe-agent, four runs for the Luna rows. Its Luna pass@1 is 0.1128 at medium versus 0.6719 at max. These are long-horizon engineering tasks, not a general judgment about every use. Network/provider/verifier errors are excluded; task timeouts and context failures count as failures. Close scores and overlapping intervals do not establish a universal ordering.

[OpenAI's July 9 release](https://openai.com/index/gpt-5-6/) reports vendor results under its stated evaluation settings. [Artificial Analysis' coding-agent comparison](https://artificialanalysis.ai/agents/coding-agents/comparisons/claude-code-vs-codex) measures an agent/model combination. Those sources help triangulate workload and harness sensitivity; their composite indices must not be substituted for per-effort DeepSWE pass@1.

The requested `GET https://openrouter.ai/api/v1/benchmarks?source=openrouter` returned HTTP 401 without authentication on September 12. Automatic approval review blocked using a stored key; no authenticated result or score was obtained. The [current official Data API documentation](https://openrouter.ai/docs/cookbook/administration/data-api) documents `source=openrouter`, classic GPQA/tau-bench and search benchmarks. Search rows combine same-configuration runs and publish each model's best eligible configuration; `include_run_config=true` adds effort, temperature and turn limit. Preserve the returned source, date, metric, task count and configuration. Search accuracy/F1 is not a coding score, and rows without effort cannot resolve medium versus max. No scores from that endpoint were added to routing.
