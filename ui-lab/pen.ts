...
export function surfacesToPen(surfaces: PenSurface[], title: string): string {
  ...
  const doc = {
    ...
    children: [
      { type: "note", id: id("n"), name: "About", x: 0, y: -80, content: `${title}. Generated by ui-lab from the real OpenCode2 Quest components; one frame per terminal row, one text layer per colour run. Re-run bun run ui:lab to regenerate.`, fontSize: 14, fill: "#8A8A8A" },
      ...children,
    ],
  }
